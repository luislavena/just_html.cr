...
describe JasperHTML::Serializer do
  describe ".to_html" do
    ...
    it "serializes void elements without closing tag" do
      doc = JasperHTML::TreeBuilder.parse("<p>Line 1<br>Line 2</p>")
      html = JasperHTML::Serializer.to_html(doc)
    ...
    it "serializes attributes" do
      doc = JasperHTML::TreeBuilder.parse("<a href=\"http://example.com\" class=\"link\">click</a>")
      html = JasperHTML::Serializer.to_html(doc)
    ...
    it "escapes special characters in text" do
      el = JasperHTML::Element.new("p")
      el.append_child(JasperHTML::Text.new("<script>alert('xss')</script>"))
      html = JasperHTML::Serializer.to_html(el)
    ...
    it "escapes special characters in attributes" do
      el = JasperHTML::Element.new("div", {"data-value" => "a<b&c\"d"})
      html = JasperHTML::Serializer.to_html(el)
    ...
    it "serializes comments" do
      doc = JasperHTML::TreeBuilder.parse("<!-- comment --><p>text</p>")
      html = JasperHTML::Serializer.to_html(doc)
    ...
    it "serializes doctype" do
      doc = JasperHTML::TreeBuilder.parse("<!DOCTYPE html><html><body></body></html>")
      html = JasperHTML::Serializer.to_html(doc)
    ...
    it "handles boolean attributes" do
      el = JasperHTML::Element.new("input", {"disabled" => nil, "type" => "text"})
      html = JasperHTML::Serializer.to_html(el)
  ...
  describe ".to_text" do
    it "extracts text from elements" do
      doc = JasperHTML::TreeBuilder.parse("<p>Hello World</p>")
      text = JasperHTML::Serializer.to_text(doc)

      text.should contain("Hello World")
    end

    it "extracts text from nested elements" do
      doc = JasperHTML::TreeBuilder.parse("<div><p>First</p><p>Second</p></div>")
      text = JasperHTML::Serializer.to_text(doc)
    ...
    it "adds newlines for block elements" do
      doc = JasperHTML::TreeBuilder.parse("<p>Para 1</p><p>Para 2</p>")
      text = JasperHTML::Serializer.to_text(doc)
    ...
    it "skips script and style content" do
      doc = JasperHTML::TreeBuilder.parse("<p>Visible</p><script>invisible</script><style>also invisible</style>")
      text = JasperHTML::Serializer.to_text(doc)
